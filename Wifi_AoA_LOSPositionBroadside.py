...
def calculate_fine_corrections(
    ...
    print("candidate_fully_corrected_phases",candidate_fully_corrected_phases)
...
def plot_aoa_pattern(corr_mags, aoa_degrees, output_path):
    """生成AoA方向图。"""
    if len(corr_mags) == 0: return
    fig = plt.figure(figsize=(10, 10))
    ...
    ax_aoa.set_theta_zero_location('N')
    ax_aoa.set_theta_direction(-1)
    ax_aoa.set_thetalim(-np.pi/2, np.pi/2)
    ax_aoa.set_title(f'Angle of Arrival (AoA) Pattern ({len(aoa_degrees)} points)', fontsize=16, pad=20)
...
# --- Step 2: Coarse CFO Estimation and Correction ---
    ...
    plot_aoa_pattern(valid_mags, valid_aoa, "aoa_pattern_filtered.png")
